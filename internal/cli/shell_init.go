...
func zshShellInitScript() string {
	lines := []string{
		...
		"_grove_complete() {",
		"  if (( CURRENT == 2 )); then",
		"    _arguments '1:command:(init add cd list ls path status remove rm version shell-init help)'",
		"    return",
		"  fi",
		"  if [[ ${words[2]} == cd ]]; then",
		"    _grove_cd_completions",
...
func bashShellInitScript() string {
	lines := []string{
		...
		"    COMPREPLY=( $(compgen -W 'init add cd list ls path status remove rm version shell-init help' -- \"$cur\") )",
		"    return 0",
		"  fi",
		"  if [ \"$prev\" = \"cd\" ]; then",
